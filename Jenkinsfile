...
int publishToPortal(String path) {
  ...
  dir(env.SOURCE_DIR) {
    ansiColor('xterm') {
      ...
      export FILENAME=$(ls ${ARCHIVE_DIR}/${BUILD_PRODUCT}_${DEVICE}-${PLATFORM_VERSION}*.zip)
      ...
      curl -X POST $BUILDS_PORTAL_URL -H "apiKey:$API_KEY" -H "Content-Type:application/json" --data-binary \
         '{ "device": "$DEVICE", "filename": "$FILENAME", "md5sum": "$MD5SUM", "romtype": "$TYPE", "url": "$OTA_URL", "version": "$PLATFORM_VERSION" }'
      ''')
    }
  }
}
