node {
    ...
    stage('Code syncing') {
        dir('/unlegacy/build_config') {
            checkout changelog: false, poll: false, scm: [$class: 'GitSCM', branches: [[name: 'refs/changes/35/1335/7']], doGenerateSubmoduleConfigurations: false, extensions: [[$class: 'WipeWorkspace']], submoduleCfg: [], userRemoteConfigs: [[refspec: '+refs/changes/*:refs/changes/*', url: 'git://github.com/Unlegacy-Android/build_config.git']]]
        }
    ...
    stage('Build process') {
        dir('/unlegacy/build_config') {
            sh returnStdout: true, script: 'bash build.sh'
        }
    }
